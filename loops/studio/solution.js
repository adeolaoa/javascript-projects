...
for (let i = 0; i < numMeals; i++ ){
  // Create a variable to store one meal array
  let meal = [];
  // Then go into the pantry, to retrieve one category at a time
  for (let j = 0; j < pantry.length; j++){
    // get specific food from each item
      meal.push(pantry[j][i]);
    }
    // put the individual meal into the meals array
    meals.push(meal);
  }
  return meals;
}
...
function askForNumber() {
  ...
  while (numMeals < 1 || numMeals > 6 || isNaN(numMeals)){
    numMeals = Number(input.question("Please enter an integer from 1-6 for the number of meals: "));
